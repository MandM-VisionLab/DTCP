...
class DTCPAgent(autonomous_agent.AutonomousAgent):
	...
	def run_step(self, input_data, timestamp):
		...
		if command < 0:
			command = 4
		command -= 1
		assert command in [0, 1, 2, 3, 4, 5]  # VOID = -1, LEFT = 1, RIGHT = 2, STRAIGHT = 3, LANEFOLLOW = 4, CHANGELANELEFT = 5, CHANGELANERIGHT = 6
		cmd_one_hot = [0] * 6
		...
		if self.status == 0:
			self.alpha = 0.3
			self.pid_metadata['agent'] = 'traj'   # see paper
