...
class TCP(nn.Module):
	...
	def forward(self, img, state, target_point):
		...
		outputs = {}
		outputs['cnn_feature'] = cnn_feature  # (B, 512, 8, 29)
		...
		cnn_ctrl_weighted = cnn_feature * init_att  # (B, 512, 8, 29)
		outputs['cnn_ctrl_weighted'] = cnn_ctrl_weighted
		feature_emb = torch.sum(cnn_ctrl_weighted, dim=(2, 3))
		...
		future_feature, future_attn_weighted, future_mu, future_sigma = [], [], [], []
		...
		for _ in range(self.config.pred_len):
			...
			weighted_feature = cnn_feature * wp_att  # (B, 512, 8, 29)
			new_feature_emb = torch.sum(weighted_feature, dim=(2, 3))
			...
			future_attn_weighted.append(weighted_feature)
			future_mu.append(mu)
			future_sigma.append(sigma)


		outputs['future_feature'] = future_feature
		outputs['future_attn_weighted'] = future_attn_weighted
