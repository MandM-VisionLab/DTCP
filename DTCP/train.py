...
from DTCP.data import CARLA_Data
from DTCP.config import GlobalConfig
from DTCP.FeatureDiversityLoss import FeatureDiversityLoss



class DTCP_planner(pl.LightningModule):
	def __init__(self, config, lr):
		super().__init__()
		self.save_hyperparameters()
		self.lr = lr
		self.config = config
		self.div = FeatureDiversityLoss()
	...
	def training_step(self, batch, batch_idx):
		...
		div_loss = self.div(pred['cnn_ctrl_weighted']) * self.config.div_weight
		...
		for i in range(self.config.pred_len):
			...
			future_div_loss += self.div(pred['future_attn_weighted'][i]) * self.config.div_weight
		future_feature_loss /= self.config.pred_len
		future_div_loss /= self.config.pred_len
		future_action_loss /= self.config.pred_len

		loss = action_loss + speed_loss + value_loss + feature_loss + div_loss + wp_loss + future_feature_loss + future_action_loss + future_div_loss
		...
		self.log('train_div_loss', div_loss.item())
		self.log('train_future_feature_loss', future_feature_loss.item())
		self.log('train_future_div_loss', future_div_loss.item())
	...
	def validation_step(self, batch, batch_idx):
		...
		self.log('val_div_loss', div_loss.item(), sync_dist=True)
		...
		self.log('val_future_div_loss', future_div_loss.item(), sync_dist=True)
...
if __name__ == "__main__":
	...
	parser.add_argument('--id', type=str, default='DTCP', help='Unique experiment identifier.')
	...
	parser.add_argument('--seed', type=int, default=42, help='Random seed during training.')
	...
	DTCP_model = DTCP_planner(config, args.lr)

	checkpoint_callback = ModelCheckpoint(save_weights_only=False, mode="min", monitor="val_loss", save_top_k=3, save_last=True,
